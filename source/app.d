...
unittest{
	...
	assert(points[0] == [0, 0.4474427700042725, 0, 0.9130669832229614, 0.9997519850730896]);
	assert(points[$-1] == [3.31227707862854, 0.9355245232582092, 0.4000000059604645, 0.9130669832229614, 0.9997519850730896]);
}
+/
...
void benchmarkNearest(){
	...
	static double nearestNaive(size_t k, T)(T[k][] points, const T[k][] testPoints){
	...
	static double nearestKdTree(size_t k, T)(T[k][] points, const T[k][] testPoints){
	...
	static double nearestKdTreeAdd(size_t k, T)(T[k][] points, const T[k][] testPoints){
	...
	auto results = benchmark!(
		{ sum1 += nearestNaive(points, testPoints); },
		{ sum2 += nearestKdTree(points, testPoints); },
		{ sum3 += nearestKdTreeAdd(points, testPoints); },
	)(10);
	assert(sum2 == sum1);
	assert(sum3 == sum1);
...
void benchmarkGrowing(){
	...
	double sum1 = 0, sum2 = 0, sum3 = 0;
	auto results = benchmark!(
		{ sum1 += nearestNaive(points); },
		{ sum2 += nearestAdd(points); },
		{ sum3 += nearestAddRebalance(points); },
	)(1);
	assert(sum2 == sum1);
	assert(sum3 == sum1);
